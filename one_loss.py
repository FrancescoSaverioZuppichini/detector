...
class MinCostMatcher(nn.Module):
    ...
    def __init__(
        self, cfg, cost_class: float = 1, cost_bbox: float = 1, cost_giou: float = 1
    ):
        """Creates the matcher
        ...
        self.head_type = cfg.MODEL.OneNet.HEAD
        assert (
            self.head_type == "CenterNet"
            or self.head_type == "RetinaNet"
            or self.head_type == "FCOS"
        )
        self.pre_define = cfg.MODEL.OneNet.PRE_DEFINE
        self.object_sizes_of_interest = torch.tensor(
